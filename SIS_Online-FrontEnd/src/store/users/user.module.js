...
const getters = {
  ...
  getUserType: state => state.type, // adicionei para filtrar por tipo de perfil (teacher/student)
  getUserProgramID: state => id => {
    const program = state.users.find(user => user._id === id);
    return program.name;
  }
};
